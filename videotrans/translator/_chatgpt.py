...
class ChatGPT(BaseTrans):
    ...
    def llm_segment(self, srt_list, ai_type='openai'):
        ...
        max_speech_s=str(settings.get('max_speech_duration_s',6))
        prompts_template=prompts_template.replace('{max_speech_s}',max_speech_s)

        chunk_size = int(settings.get('llm_chunk_size', 20))
        api_key = params.get('chatgpt_key','') if ai_type == 'openai' else params.get('deepseek_key','')
        model_name = params.get('chatgpt_model','') if ai_type == 'openai' else params.get('deepseek_model','')
        api_url = self._get_url( params.get('chatgpt_api','')) if ai_type == 'openai' else 'https://api.deepseek.com/v1'
    ...
    def _item_task(self, data: Union[List[str], str]) -> str:
        ...
        response = model.chat.completions.create(
            model=params.get('chatgpt_model',''),
            timeout=300,
            frequency_penalty=0,
            temperature=float(settings.get('aitrans_temperature',0.2)),
